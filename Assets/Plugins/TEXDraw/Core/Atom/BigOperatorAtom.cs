...
namespace TexDrawLib
{
    public class BigOperatorAtom : ScriptsAtom
    {
        ...
        {
            if (!limitMode || (subscript == null && superscript == null))
            {
            ...
            if (superscript is SymbolAtom atom1)
                upperLimitBox = atom1.CreateBoxMinWidth(baseBox.width, state);
            else
                upperLimitBox = superscript?.CreateBox(state);

            if (subscript is SymbolAtom atom2)
                lowerLimitBox = atom2.CreateBoxMinWidth(baseBox.width, state);
            else
            ...
            var baseBoxHeight = baseBox.TotalHeight / 2 + median;
